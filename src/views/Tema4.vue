<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5
    .titulo-principal(data-aos="fade-right")
      .titulo-principal__numero
        span 4
      h1 Elaboración de muestra física sudadera
    p(data-aos="fade-down") La realización de operaciones para la ropa deportiva se caracteriza por los tipos de tejidos utilizados para la elaboración de este tipo de prendas, donde algunos tienen más elongación por lo que requieren de máquinas, aditamentos, accesorios y ajustes especializados para el trabajo.
    p(data-aos="fade-down") Una de las prendas de mayor reconocimiento en la ropa deportiva es el pantalón de sudadera, a continuación, conoceremos cada una de las operaciones.

    LineaTiempoD.color-primario.mt-5(data-aos="flip-up")
      .row.justify-content-center.align-items-center.time-bg-1(numero="1" titulo="Paso 1. Alistamiento para confección de pantalón de sudadera.")
        .col-lg-12.mt-4.time-bg-1
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/G-cwBVckDq4" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="2" titulo="Paso 2. Medir, cortar, coser elástico y marcar cuartas partes de elástico.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/uuWDflzDFGA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="3" titulo="Paso 3. Coser bolsillos laterales.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/Zwfp0oWJftk" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="4" titulo="Paso 4. Filetear bolsillos.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/Mie5EoCZVMw" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="5" titulo="Paso 5. Unir costados incluyendo sesgo de embone.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/9A3TteZcgno" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="6" titulo="Paso 6. Filetear costado.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/9ph3Sel-9Bs" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="7" titulo="Paso 7. Unir por entrepierna.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/79XxGyuhmQg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="8" titulo="Paso 8. Unir tiros.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/MriPZnacsDI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="9" titulo="Paso 9. Fijar elástico.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/Eg9HQYZVyKA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="10" titulo="Paso 10. Dobladillar botas.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/0qf5jZGpnZo" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="11" titulo="Paso 11. Sentar elástico.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/qVoGjoahb7g" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

      .row.justify-content-center.align-items-center(numero="12" titulo="Paso 12. Limpiar y revisar prenda.")
        .col-lg-12.mt-4
          figure
            .video
              iframe(width="560" height="315" src="https://www.youtube.com/embed/3ddl46WHcDU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen)

    .row.justify-content-center.align-items-center 
      .col-lg-8
        .tarjeta.color-primario.call-bg.p-3.mt-5
          .row.justify-content-around.align-items-center
            .col-3.col-sm-2.col-lg-1
              img(src="@/assets/template/icono-xls.svg")
            .col
              .row.justify-content-between.align-items-center
                .col.mb-3.mb-sm-0
                  h4.mb-1 Anexo
                  p.text-small Ficha técnica para elaboración de una sudadera. 
                .col-sm-auto
                  a.boton.color-acento-contenido.texto-blanco(:href="obtenerLink('/downloads/CF16_B_4_FICHA_TECNICA_SUDADERA_DEPORTIVA.xlsx')" target="_blank")
                    span Descargar
                    i.fas.fa-file-download              

</template>

<script>
export default {
  name: 'Tema4',
  data: () => ({
    // variables de vue
  }),
  mounted() {
    this.$nextTick(() => {
      this.$aosRefresh()
    })
  },
  updated() {
    this.$aosRefresh()
  },
}
</script>

<style lang="sass" scoped></style>
